...
import
{
    ...
    faFilePowerpoint, faFileZipper, faFileLines, faFile
...
const RightSidebar = () =>
{
    ...
    const MediaThumb = ( { msg, index, isOverlay = false, overlayCount = 0 } ) => (
        ...
        >
            { msg.type === "IMAGE" ? (
                <img src={ msg.content } alt="media" className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-200" />
            ) : (
    ...
    return (
        <>
            <div className="h-full w-full px-4 py-6 border-[rgba(200,200,200,.5)] border-l overflow-y-auto relative">
                ...
                { !hasAccess ? (
                    <div className="flex flex-col items-center justify-center py-16 text-center">
                        <img className="w-6 h-6 mb-1 mx-auto opacity-80" src={ lockIcon } alt="" />
                        <p className="text-[13px] font-semibold text-gray-700">Restricted</p>
                        <p className="text-[11px] text-gray-400 mt-1">
                            Only project members can view details.
                        </p>
                    </div>
                ) : (
                    <>
                        ...
                        <div className="border-b border-[rgba(200,200,200,.5)] pb-5 mb-4">
                            <div className="flex justify-between items-center mb-4">
                                ...
                                { mediaMessages.length > PREVIEW_LIMIT && (
                                    <button onClick={ () => openAllMedia( 0 ) } className="text-[11px] opacity-50 hover:opacity-100 transition-opacity">
                                        See all
                                    </button>
                                ) }
                            </div>
                        ...
                        <div>
                            <div className="flex justify-between items-center mb-4">
                                ...
                                { fileMessages.length > FILE_PREVIEW_LIMIT && (
                                    <button onClick={ () => setShowAllFiles( true ) } className="text-[11px] opacity-50 hover:opacity-100 transition-opacity">
                                        See all
                                    </button>
                                ) }
                            </div>
                                        ...
                                        >
                                            ...
                                            <div className="flex-1 min-w-0">
                                                ...
                                                <p className="text-[11px] opacity-40">
                                                    { msg.sender?.fullName || "User" } • { msg.sentAt ? new Date( msg.sentAt ).toLocaleDateString() : "" }
                                                </p>
                                            </div>
            ...
            { selectedMedia && (
                <div className="fixed inset-0 bg-black/80 z-50 flex flex-col items-center justify-center" onClick={ () => setSelectedMedia( null ) }>
                    <button className="absolute top-4 right-4 text-white text-2xl font-bold z-10" onClick={ () => setSelectedMedia( null ) }>✕</button>
                    <div className="absolute top-4 left-4 text-white">
                        <p className="text-sm font-medium">{ selectedMedia.sender?.fullName || "User" }</p>
                        <p className="text-xs opacity-50">{ selectedMedia.sentAt ? new Date( selectedMedia.sentAt ).toLocaleString() : "" }</p>
                    </div>
                    <div className="max-w-[90vw] max-h-[80vh]" onClick={ ( e ) => e.stopPropagation() }>
                        ...
                        { selectedMedia.caption && <p className="text-white text-sm text-center mt-3 opacity-80">{ selectedMedia.caption }</p> }
                    </div>
                    <Link to={ selectedMedia.content } target="_blank" rel="noreferrer" onClick={ ( e ) => e.stopPropagation() }
            ...
            { showAllMedia && (
                <div className="fixed inset-0 bg-black/90 z-50 flex flex-col" onClick={ () => setShowAllMedia( false ) }>
                    <div className="flex items-center justify-between px-5 py-4" onClick={ ( e ) => e.stopPropagation() }>
                        <h3 className="text-white font-medium">All Media <span className="ml-2 opacity-50 text-sm">({ mediaMessages.length })</span></h3>
                        <button className="text-white text-2xl font-bold" onClick={ () => setShowAllMedia( false ) }>✕</button>
                    </div>
                    <div className="flex-1 flex items-center justify-center px-4 overflow-hidden" onClick={ ( e ) => e.stopPropagation() }>
                        { mediaMessages[ allMediaIndex ]?.type === "IMAGE" ? (
                            <img src={ mediaMessages[ allMediaIndex ].content } alt="full view" className="max-w-full max-h-full rounded-lg object-contain" />
                        ) : (
                            <video src={ mediaMessages[ allMediaIndex ]?.content } controls autoPlay className="max-w-full max-h-full rounded-lg" />
                        ) }
                    </div>
                    <div className="flex gap-2 px-4 py-3 overflow-x-auto" onClick={ ( e ) => e.stopPropagation() }>
                        { mediaMessages.map( ( msg, index ) => (
                            <div key={ index } onClick={ () => setAllMediaIndex( index ) }
                                ...
                                { msg.type === "IMAGE" ? (
                                    ...
                                    <div className="w-full h-full bg-gray-700 flex items-center justify-center"><span className="text-white text-xs">▶</span></div>
                                ) }
                            </div>
                        ) ) }
                    </div>
            ...
            { showAllFiles && (
                <div className="fixed inset-0 bg-black/80 z-50 flex flex-col" onClick={ () => setShowAllFiles( false ) }>
                    <div className="bg-white w-full max-w-md mx-auto mt-auto rounded-t-2xl flex flex-col max-h-[80vh]" onClick={ ( e ) => e.stopPropagation() }>
                        <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100">
                            <h3 className="font-medium text-[14px]">All Files <span className="ml-2 opacity-50 text-[12px]">({ fileMessages.length })</span></h3>
                            <button className="text-gray-500 text-xl cursor-pointer font-bold" onClick={ () => setShowAllFiles( false ) }>✕</button>
                        </div>
                        <div className="overflow-y-auto px-4 py-3 space-y-2">
                            { fileMessages.map( ( msg, index ) => (
                                <Link key={ index } to={ msg.content } target="_blank" rel="noreferrer"
                                    className="flex items-center gap-3 p-2.5 rounded-lg bg-[#F5F5F5] hover:bg-[#EBEBEB] transition-colors group">
                                    <div className="w-8 h-8 rounded-md bg-white flex items-center justify-center text-lg shadow-sm shrink-0">
                                        { getFileIcon( msg.fileName ) }
                                    </div>
                                    <div className="flex-1 min-w-0">
                                        <p className="text-[12px] font-medium truncate">{ msg.fileName || "File" }</p>
                                        <p className="text-[11px] opacity-40">{ msg.sender?.fullName || "User" } • { msg.sentAt ? new Date( msg.sentAt ).toLocaleDateString() : "" }</p>
                                    </div>
                                    <span className="text-[11px] opacity-0 group-hover:opacity-40 transition-opacity">⬇</span>
                                </Link>
                            ) ) }
